...
#'   since occasions are a random facet however the raters are treated. At
#'   `n_o` = the fitted occasion count it matches the fitted `ICC(*,k)`.
#'
...
tidy.icc_dstudy <- function(x, ...) {
  ...
  n <- length(x$m)
  # `x` is a data frame, so a missing column has to be tested by name: `$` on one
