...
#' number of raters `m`. This is a generalizability-theory **decision (D-)
#' study**, answering "how reliable would the mean of `m` raters be?" and, read
#' as a curve, "how many raters do I need?". The point estimate and its
#' boundary-aware interval reuse the fit stored on `x`, and no model is refit.
#' The
#' band follows the fit's `ci_method`. A Monte-Carlo fit reprojects one draw
#' from the parameter covariance across every `m`. A **bootstrap** fit
#' reprojects its stored resamples, so at `m` = the observed rater count the
#' band matches the fitted `ICC(*,k)` interval exactly.
#'
#' @section Projection is extrapolation:
#' Projecting to an `m` you did not run is an **extrapolation**. Its
#' trustworthiness depends on how well the variance components are pinned down,
#' especially the rater variance \eqn{\sigma^2_r}, which is estimated from only
#' as many raters as you observed. With few raters that estimate is noisy, so
#' the projected interval is honestly wide. The Monte-Carlo interval widens
#' automatically, recomputing \eqn{\Phi(m)} on every draw rather than
...
#' agreement. There the rater term is the finite-population variance of exactly
#' the raters you observed, so there is no "average of `m` freshly sampled
#' raters" to project to. `d_study()` aborts in that case (use
#' `raters = "random"`).
#'
...
#'   at the observed count: pure error divides by `m * n_o` while the rater and
#'   interaction terms are unchanged. Because occasion averaging rescales **only pure
#'   error**, this curve is well-posed for random **and** fixed raters. That
#'   includes fixed absolute agreement, which the rater projection refuses,
#'   since occasions are a random facet however the raters are treated. Where
#'   the swept `n_o` equals the fitted occasion count it matches the fitted
#'   `ICC(*,k)`.
#'
...
#' **rater** projection moves the subject level across occasion settings and the
#' cluster level single-occasion. An **occasion** projection moves the subject
#' level across `n_o` and returns the cluster level as a **flat** curve. The
#' cluster-level error set (`{rater, cluster:rater}`) has no pure-error term, so
#' averaging occasions cannot change it, and `d_study()` notes this. **Ragged**
...
#'   exclusive with `m`, and supplying both aborts. `NULL` (the default)
...
#'     that table.
#'   * `autoplot.icc_dstudy()`: a `ggplot` object holding the reliability curve,
