...
#
# What is NOT named, and why the wording is what it is. Every method named here is a
# DETERMINISTIC closed form, and D-012 measured SEARLE and Burch returning a finite
# interval on 100% of 32,000 balanced one-way datasets where the MC default aborted
# on 4-44%; each method's parenthetical is D-012's own one-line characterization.
# `ci_method = "npbootstrap"` is named on NO design, which also empties the
# unbalanced one-way row (it is the only method shipping that cell, D-013): its
# second abort is a RESAMPLE-stage guard (`R/ci-npbootstrap.R:178-191`) that is
# stochastic and not a property of the observed data, so a single run says something
# about that seed rather than about the design. M97 takes it up behind this same
# verification plus the RNG-neutrality and seed questions it alone raises.
...
boundary_method_hint <- function(
  ...
  usable <- function(method) {
    boundary_method_usable(method, df, estimands, conf_level, n0)
  }
  ...
  if (isTRUE(oneway)) {
    ...
    # This is ADMISSIBILITY: `icc()` refuses the string here, so no amount of
    # verification could license naming it.
    if (!isTRUE(balanced)) {
      return(character(0))
    }
