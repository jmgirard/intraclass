...
| M62 | Non-parametric bootstrap CI pass — one-way ICC (GO/NO-GO) | in-progress | — | normal | milestones/M62-ci-method-comparison-pass.md |
...
- Exported non-parametric bootstrap / profile-likelihood `ci_method` — the GO-gated implementation of whichever method M62 (bootstrap) / its PL sibling finds "not worse" than the incumbents. Promote per method only on a GO; a NO-GO is a recorded D-entry rejection, not re-litigated. Lineage: legacy candidate → M62 + PL sibling.
- REFERENCES.md → cairn-style split: migrate the per-primary-source extractions into `<citekey>.md` source notes (convention already started — ukoumunne2003, ohyama2025) and keep the ~40-entry oracle registry as a single synthesis/registry page (candidate: `ORACLES.md`) + the bibliography. NOT a file-per-paper shred — the file is ~90% oracle→provenance registry and tests cite oracle IDs (~19 citing files). Reconcile with the recorded cairn-side ORACLES.md open question (DESIGN.md § Known issues; cairn D-024/M42). Sequence after M48 (churn on a high-traffic file, no pre-1.0 user benefit). Promote via `/milestone-plan`. Lineage: raised 2026-07-17 during M62 source ingestion.
