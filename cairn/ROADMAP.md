...
|---|---|---|---|---|---|
| M53 | Multilevel SEM (lavaan) — estimand/oracle pass | in-progress | — | high | milestones/M53-multilevel-sem-pass.md |
...
- Multilevel SEM (lavaan) engine implementation — `engine = "lavaan"` + `cluster`, Design 1, both levels, complete/balanced, random raters, montecarlo CI. **M53 verdict: GO (2026-07-16)** — the two-level formulation is oracle-established (D-005; pilot ledger `cairn/references/sem-multilevel-pilot.md`); schedulable via `/milestone-plan`. Still **blocks** the lavaan cluster-level-fixed and incomplete-fixed-nested siblings. Lineage: reclassified from the M21 SEM-parity plan (ADR-027); promoted from the parking lot 2026-07-13 — cairn/legacy/ROADMAP.md
