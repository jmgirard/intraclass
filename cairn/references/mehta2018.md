...
| **Near-zero ICC** | Reached, at last. Case 6 produces `ICC` = 0.08 (extreme convex, `N = 300`), the **lowest true ICC anywhere in the M65 cluster** — recomputed over the cluster's design minima: `mehta` 0.08 < `xiao2009` ρ 0.1 = `bhandary2006` ρ 0.1 < `bobak2018` 0.295 < `donner2002` ρ 0.4 < `xiao2013` ρ 0.6. (`saha2005`/`saha2012`'s smallest `φ` is 0.05, numerically lower, but `φ` is a binary beta-binomial dispersion parameter, not a Gaussian ICC, so it is not a "true ICC" in the same sense.) The paper reports no estimation difficulty there — unsurprising, since it uses closed-form mean squares that cannot fail to converge, unlike the package's mixed-model fits |
