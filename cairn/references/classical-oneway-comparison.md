...
**The width margin is conditional, and its shape is not the obvious one** — a
second reading corrected at M117, one level down from the M116 one above.
Replacing "Burch is wider" with a single pooled figure per grid ("about 6% and
about 4% narrower") stated as a constant what the grids measure as a function of
two design factors. Recomputed from the same fixture's per-factor block: on the
...
not quoted either: n=2 occurs only at k=10 in both grids, so both the marginal
