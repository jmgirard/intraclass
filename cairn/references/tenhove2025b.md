...
online 16 Jun 2025. PDF: `cairn/references/sources/tenhove2025b.pdf` (gitignored).
...
- **MLE-RE** — maximum likelihood estimation of random-effects models
  (Marcoulides 1990); same underlying model as MCMC-HL, Eqs. 1–2 (p. 1048).
...
(Wald) CIs** (`car::deltaMethod()`) and **Monte-Carlo CIs**
(`semTools::monteCarloCI()`) for both MLE-RE and MLE-CF; **percentile-based
BCIs** for MCMC-HL; inverted-*F* for ANOVA. Figure 7 (p. 1054) labels these
...
- **Scaling of other ICCs (p. 1050).** Only `ICC(A,1)` and `ICC(C,1)` were
  ...
  `k`, `k̂` or `q`", so they "only proportionally influence the computational
  accuracy" (p. 1050).
...
- **Point-estimate bias (Fig. 3, p. 1051; Fig. 4, p. 1052).** "Across all
  ...
  ICC(A,1) but underestimated ICC(C,1) at 80% missing. ANOVA "consistently
  underestimated the ICC(C,1)" (p. 1051), as predicted (p. 1050) because it
  cannot separate `σ²_r` from `σ²_sr`.
- **Variability (SE) bias (Figs. 5–6, pp. 1052–1053).** "Across conditions,
  ...
  shows relative bias of the `σ²_r` variability estimate reaching roughly +500%
  to +600% (plot-read, approximate). MLE-CF substantially *under*estimated the
  variability of `σ²_r`.
- **Coverage (Fig. 7, p. 1054).** "Across conditions, MLE-RE with Monte-Carlo
  ...
  Monte-Carlo CIs" (p. 1053). ANOVA coverage was too low for both ICCs, improving
...
- **Small rater samples (p. 1057, Discussion).** "The coverage rates of the
  interrater agreement were considerably lower for small samples of raters"
  (p. 1057) — because ICC(A,·) carries `σ²_r` in the denominator and it is
...
4. **Supplementary Materials A and B are not in the shelf PDF.** The complete
...
6. `ORACLES.md` does not appear to cite this paper as an oracle for any entry
   (grep for "ten Hove" returns 2020/2022 entries only), which is consistent with
   its ADR-only role. No disagreement found.
