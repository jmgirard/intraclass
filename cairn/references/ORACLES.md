...
### Oracle O-NPBoot — one-way transformed bootstrap-t interval, `ci_method = "npbootstrap"` (M75, D-006/D-010; unbalanced ICC(1) M84, unbalanced ICC(k) M85)
- **Kind:** mixed — external leg: the ukoumunne2003 Table I exact coverage + the ohyama2025 Fig. 2 / Example 2 (unbalanced); script leg: the RR01-verified prototype (parity) + the shipped-reducer coverage sweeps.
- **Status:** **asserted (M75)** in `tests/testthat/test-ci-npbootstrap.R` (parity + guards + ICC(k) identity) and `tests/testthat/test-ci-npbootstrap-coverage.R` (the committed sweep fixture). The exported balanced one-way transformed bootstrap-t (ukoumunne2003 eq. 6/7; whole-subject resample → `log F` transform → IJ SE → studentized, back-transformed, untruncated). Five oracles pin it:
  1. **Prototype parity (AC2)** — the exported reducer reproduces the RR01-verified prototype `data-raw/m62-npbootstrap-prototype.R` transformed bootstrap-t ICC(1) endpoints to ≥ 4 dp (identical resample stream given the seed), on the committed fixture `fixtures/npbootstrap-parity-oracle.rds`.
  2. **ukoumunne2003 Table I external (AC3)** — two-sided ICC(1) coverage at (k∈{10,30,50}, n=10, ρ=.05) matches the exact values **0.938 / 0.944 / 0.9395** within a pre-registered ±0.03, from the n_rep ≥ 2000 sweep.
  3. **Tail balance (AC4)** — the near-zero cells keep a roughly even lower/upper tail split (tracking Table I's ~3.25/2.95 at U10, not a BCa-style skew), and truncated-vs-untruncated widths are both reported.
  4. **ICC(k) inheritance (BC2/BC3), NOT an independent anchor** — the `unit = "average"` interval is the exact monotone Spearman-Brown image `g(ρ)=kρ/(1+(k−1)ρ)` of the ICC(1) interval; the composed map is `1 − e^(−logF)` (= the classical ANOVA ICC(k)). Its validation basis is **exact monotone-map inheritance from the ICC(1) Table I anchor + the BC2 identity cross-check** (`ICC(k) endpoint = 1 − exp(−logF endpoint)`, ≤ 1e−10, doubling as a `k_eff = n` design-consistency guard) — it has **no independent external anchor**, and coverage inheritance is enforced rep-by-rep in the sweep (BC3, zero discrepant reps).
  5. **Point-outside bound (BC6)** — the reported (engine REML) point-outside-interval rate is bounded by the recorded upper-tail error at every cell with ρ > 0 (an exact logical subset, RR02 §3d).
- **Unbalanced ICC(1) extension (M84, MD-1):** the reducer generalizes to unequal `n_i` — the ANOVA effective size `n0` (ohyama2025 eq. 3) in the `log F` transform, the per-`n_i` eq. 7 IJ SE, and the `C`-dropped studentization `theta = log(SSA) − log(SSE)` the SE is derived for (ukoumunne2003 Appendix A A4–A5; Form A). Three legs pin the unbalanced `unit = "single"` interval (`test-ci-npbootstrap-unbalanced.R` + `-coverage.R`):
  a. **Reduces-to-balanced (exact)** — fed equal `n_i` the per-`n_i` reducer reproduces the committed M75 balanced endpoints to ≤ 1e−10; the M75 code is the oracle for the balanced limit (this leg is silent about genuinely-unequal-size behaviour).
  b. **Deterministic point** — the unbalanced MoM `ρ̂ = (MSA−MSE)/(MSA + (n0−1)MSE)` reproduces ohyama2025 §4 Example 2 (PaCO₂) `ρ̂ = 0.585` from its printed ANOVA table, and `npb_anova`'s `n0` matches eq. 3 (distinct from the harmonic mean).
  c. **Coverage (plot-read)** — an n_rep ≥ 2000 unbalanced sweep (`data-raw/m84-npbootstrap-unbalanced-coverage.R`, balanced + MCAR 0.1) matches the ohyama2025 Fig. 2 NBOOT coverage within a ±0.02 plot-read band at three near-nominal cells, and reproduces the few-clusters under-coverage dip at k=10 / n≥5 the paper names (p. 595). The unequal-`n_i` IJ SE has **no independent external anchor** — its basis is the Appendix-A derivation (MD-1), verified end-to-end by legs (a)+(c), not by an unbalanced Table-I-style value (ukoumunne2003 is balanced-only; ohyama2025 tabulates no NBOOT coverage numbers, only Fig. 2).
- **Unbalanced ICC(k) extension (M85, MD-1):** the `unit = "average"` interval is the monotone Spearman-Brown image `g(ρ) = k_eff·ρ/(1 + (k_eff−1)ρ)` of the unbalanced ICC(1) endpoints, with the package's harmonic-mean `k_eff` divisor. Its basis is **exact monotone-map inheritance** from the unbalanced ICC(1) legs above — **no independent external anchor** — validated (`test-ci-npbootstrap-unbalanced-icck.R` + the M85 T4 ICC(k) column in `-unbalanced-coverage.R`) by:
  d. **Pole/support alignment (AC1, GP7)** — the derived GO condition `k_eff ≤ n0` (harmonic mean ≤ ohyama eq. 3, proved for every one-way design via AM-GM on triples; MD-1), so the SB pole `−1/(k_eff−1)` never intrudes on the attainable ρ and `g` is finite + strictly monotone. Checked numerically (`min(n0−k_eff) = −1.4e−14`, 2×10⁵ random + adversarial designs).
  e. **Re-derived identity cross-check (AC4)** — the shipped SB route equals the direct construction `g(ρ) = k_eff(F−1)/(k_eff(F−1)+n0)`, `F = exp(logf)`, to ≤ 1e−10 (a second path using `k_eff` and `n0` separately, so a divisor-swap bug breaks it). Unlike balanced, this is **not** `1 − 1/F` when `k_eff ≠ n0`; it collapses to `1 − 1/F` on balanced data (RR02 BC2).
  f. **Rep-by-rep coverage inheritance (AC2)** — the M84 n_rep ≥ 2000 unbalanced sweep, extended with an ICC(k) coverage column against the true `k_eff·ρ/(1 + (k_eff−1)ρ)`, records `n_discrepant = 0` and `coverage_icck == coverage_icc1` at every cell (exact event identity, tolerance 0), so ICC(k) inherits the ICC(1) plot-read coverage exactly.
- **Decision:** exported as the one-way `ci_method = "npbootstrap"` — ICC(1) and ICC(k) on balanced **and** unbalanced data (a numeric D-study `unit = m` stays balanced-only, not pole-safe unbalanced); percentile & BCa NO-GO (D-006). Point estimate is the engine REML point for both estimands (BC5); the ANOVA-MoM ρ̂ is interval machinery only.
- **Provenance:** ukoumunne2003 Table I + ohyama2025 Fig. 2 / §4 Example 2 (external) + reproducible in-suite fixtures; regenerate with `data-raw/m75-npbootstrap-parity-oracle.R` (parity), `data-raw/m75-npbootstrap-coverage.R` (balanced sweep), and `data-raw/m84-npbootstrap-unbalanced-coverage.R` (unbalanced sweep, incl. the M85 ICC(k) column). See D-006 (GO/NO-GO), D-010/RR02 (exported-API scope), M84/MD-1 (unbalanced ICC(1)), M85/MD-1 (unbalanced ICC(k)), and `cairn/references/ukoumunne2003.md` + `ohyama2025.md`.

### Oracle O-Classical-OW — classical boundary-robust one-way ICC intervals: SEARLE exact-F + Burch REML (M76, D-012)
- **Kind:** source-traceable — both legs are deterministic published worked examples; asserted directly in the test suite against the sources' printed limits (no committed fixture needed).
- **Status:** **suite-asserted (M82).** M82 exported both intervals as `ci_method = "searle"` / `"burch"` and migrated these oracles from the M76 prototype's `stopifnot` into `tests/testthat/test-ci-classical.R`, which asserts each interval core against the published worked examples plus the two self-checks. (Historically prototype-only under M76, the GO/NO-GO assessment that shipped no code — D-012.) Two constructions, each pinned by ≥2 independent published sources:
  1. **SEARLE exact-F** — the exact-F pivot for `ICC(1)`/`ICC(k)` (mcgraw1996 Table 7 form; Burch 2011 eq. 3). Reproduces **ohyama2025 §4 Ex.1 PMOC (0.600, 0.891)** within 0.002 and **burch2011 §4 arsenic normal-based (0.81, 0.94)** within 0.005 — two independent datasets/papers — plus the mcgraw1996 Table 7 algebraic-identity self-check (≤ 1e−9).
  2. **Burch (2011) REML** — kurtosis-adjusted `log(1+nθ̂)` limits (eq. 6/13/15/16/17). Reproduces **ohyama2025 §4 Ex.1 PMOC REML (0.620, 0.885)** exactly and **burch2011 §4 arsenic REML (0.73, 0.95)** within 0.005 — two independent published sources — plus an eq.13/14/15 raw-data kurtosis-pipeline self-consistency check (mean bias-corrected κ̂̂ ≈ 0 under normality, which caught + fixed an eq.14 transcription bug).
- **Coverage characterization (not an anchor):** the M76 sweep (`data-raw/m76-sweep-results.rds`, 16 cells × 2000 reps) — both methods 0 aborts on all 32k datasets vs MC 4–44%; SEARLE near-nominal + symmetric (one leptokurtic high-`k` under-cover), Burch conservative / never-under-cover. A characterization for the GO/NO-GO, not an oracle anchor.
- **Decision:** **NO-GO for default replacement** (neither passes the frozen every-cell criterion), **GO as an opt-in `ci_method`** for both; the default stays glmmTMB MC (no `#3`/ADR-003 change). See D-012.
- **Sources / provenance:** ohyama2025 §4 (pp. 599–600) + burch2011 §4 (Table 3, p. 1027) + mcgraw1996 Table 7 (external, deterministic); asserted in `tests/testthat/test-ci-classical.R` (M82), with the original prototype `data-raw/m76-classical-oneway-prototype.R` and the coverage characterization `data-raw/m76-coverage-sweep.R` retained. See `cairn/references/burch2011.md`, `ohyama2025.md`, and `classical-oneway-comparison.md`.

### Oracle O-MPL — modified profile-likelihood two-way random ICC(A,1)/ICC(A,k)/ICC(A,m) interval, `ci_method = "mpl"` (M88, D-014/D-015; M89/D-016 numeric unit)
- **Kind:** mixed — the interval CORE is source-traceable (a deterministic published worked example, xiao2013 Ex. 1); the `κ_m` correction constant is **script-derived (D-008)** below `ρ = 0.6`, where xiao2013 gives no value (established by simulated coverage, not an external anchor).
- **Status:** **suite-asserted (M88)** in `tests/testthat/test-ci-mpl.R`. M88 exported the xiao2013 interval as `ci_method = "mpl"` (the deterministic machinery ported from the M86 prototype into `R/ci-mpl.R`) and the `κ_m` table into `R/sysdata.rda`. Legs:
  1. **Deterministic core (AC1)** — the ported `mpl_interval()` reproduces xiao2013 **Ex. 1** (p. 2255): point ρ̂ = 0.8987 (to 1e−3), naive-PL 90% two-sided upper 0.9598 (to 5e−3), lower 0.7120 (to ~0.012 — the published summary inputs are rounded, so the small-`S` lower is not forced to the digit, #4). Asserted in-suite.
  2. **MC coverage / `κ_m` (offline anchor)** — the M86 harness (`data-raw/m86-mpl-validate.R`) reproduces xiao2013 **Tables 3/4/6/7** (coverage CR ±30/1000, length ±0.05, published-`κ_m` Table 3 ±0.10). Inherently MC; the committed offline oracle a fast suite cannot re-run (M88 AC1).
  3. **ICC(A,k) / numeric-`unit` ICC(A,m) inheritance (AC3; M89)** — the `unit = "average"` interval is the exact monotone Spearman-Brown image `k·ρ/(1+(k−1)ρ)` of the ICC(A,1) endpoints (McGraw & Wong 1996 Table 4; two-way random absolute agreement divides both σ²_r and σ²_e by `k`), and a numeric `unit = m` (D-study projection ICC(A,m)) is the same image at divisor `m`. **Pole-safe for `m ≥ 1`** — the SB pole `ρ = −1/(m−1)` is negative while the MPL endpoints are clamped to `[0, 1]`, so the map stays monotone in `[0, 1]` and needs no fence (unlike the unbalanced one-way npbootstrap case, where `m > n0` pushes the pole interior; M89/D-016). **No independent external anchor** — inheritance, as O-NPBoot/O-Classical-OW; asserted by the SB-identity + wrong-divisor mutation check.
  4. **`κ_m` table (M88 T3)** — `data-raw/m88-mpl-kappa-table.R` regenerates `κ_m` per (R, S) over `ρ ∈ [0.05, 0.9]` (M87 T2's per-geometry scan + top-3 bias-corrected re-eval); cross-checked against M87's committed `data-raw/m87-kappa-recalibration.rds` at the four shared geometries.
- **Decision:** **GO for an opt-in `ci_method`, NOT a default replacement** (D-014): near-nominal-or-conservative coverage across the pre-registered grid, an interval on 100% of datasets where the two-way MC default aborts (25.9–31.2% at the boundary), but over-covers + ~24% wider at interior cells. Balanced-complete two-way random absolute-agreement, Gaussian, `conf_level` ∈ {0.90, 0.95, 0.99} (*corrected M133*: this line read `conf_level = 0.95` only, the M88 fence; M91/D-017 extends D-014/D-015 to 0.90 and 0.99 ("supersedes nothing", its own Consequences) and all three levels ship, no level deeper authorized). See D-014 (GO/NO-GO), D-015 (exported-API scope), D-017 (the level set).
- **Sources / provenance:** xiao2013 Tables 3/4/6/7 + Ex. 1 (external) + the M88 `κ_m` table (script-derived, no external oracle below ρ = 0.6, D-008); asserted in `tests/testthat/test-ci-mpl.R` (M88), with the M86 prototype (`data-raw/m86-mpl-lib.R`, `-validate.R`) and M87 GO/NO-GO scripts retained. See `cairn/references/xiao2013.md` and `mpl-twoway-random-comparison.md`.
