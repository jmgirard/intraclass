...
### Oracle O-NPBoot — one-way transformed bootstrap-t interval, `ci_method = "npbootstrap"` (M75, D-006/D-010; unbalanced ICC(1) M84)
...
- **Decision:** exported as the one-way `ci_method = "npbootstrap"` — balanced ICC(1)/ICC(k), unbalanced ICC(1); percentile & BCa NO-GO (D-006). Point estimate is the engine REML point for both estimands (BC5); the ANOVA-MoM ρ̂ is interval machinery only.
- **Provenance:** ukoumunne2003 Table I + ohyama2025 Fig. 2 / §4 Example 2 (external) + reproducible in-suite fixtures; regenerate with `data-raw/m75-npbootstrap-parity-oracle.R` (parity), `data-raw/m75-npbootstrap-coverage.R` (balanced sweep), and `data-raw/m84-npbootstrap-unbalanced-coverage.R` (unbalanced sweep). See D-006 (GO/NO-GO), D-010/RR02 (exported-API scope), M84/MD-1 (unbalanced), and `cairn/references/ukoumunne2003.md` + `ohyama2025.md`.
