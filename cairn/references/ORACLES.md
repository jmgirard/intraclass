...
- **Decision:** **GO for an opt-in `ci_method`, NOT a default replacement** (D-014): near-nominal-or-conservative coverage across the pre-registered grid, an interval on 100% of datasets where the two-way MC default aborts (25.9–31.2% at the boundary), but over-covers + ~24% wider at interior cells. Balanced-complete two-way random absolute-agreement, Gaussian, `conf_level = 0.95` only. See D-014 (GO/NO-GO), D-015 (exported-API scope).
