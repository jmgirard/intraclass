...
### Oracle O-MPL — modified profile-likelihood two-way random ICC(A,1)/ICC(A,k) interval, `ci_method = "mpl"` (M88, D-014/D-015)
...
- **Status:** **suite-asserted (M88)** in `tests/testthat/test-ci-mpl.R`. M88 exported the xiao2013 interval as `ci_method = "mpl"` (the deterministic machinery ported from the M86 prototype into `R/ci-mpl.R`) and the `κ_m` table into `R/sysdata.rda`. Legs:
  ...
  3. **ICC(A,k) inheritance (AC3)** — the `unit = "average"` interval is the exact monotone Spearman-Brown image `k·ρ/(1+(k−1)ρ)` of the ICC(A,1) endpoints (McGraw & Wong 1996 Table 4; two-way random absolute agreement divides both σ²_r and σ²_e by `k`). **No independent external anchor** — inheritance, as O-NPBoot/O-Classical-OW; asserted by the SB-identity + wrong-divisor mutation check.
