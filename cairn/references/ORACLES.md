...
- **Kind:** source-traceable — both legs are deterministic published worked examples; validated by the data-raw prototype's `stopifnot`, not by a committed fixture.
- **Status:** **prototype-validated, NOT suite-asserted.** M76 is a GO/NO-GO assessment that ships **no exported code** (D-012), so these oracles are asserted in `data-raw/m76-classical-oneway-prototype.R` (via `stopifnot`), not in `tests/testthat/`. The follow-on opt-in `ci_method` milestone (D-012 recommendation) asserts them in the suite when it ships the exported reducer. Two constructions, each pinned by ≥2 independent published sources:
...
- **Sources / provenance:** ohyama2025 §4 (pp. 599–600) + burch2011 §4 (Table 3, p. 1027) + mcgraw1996 Table 7 (external, deterministic); validated by `data-raw/m76-classical-oneway-prototype.R`, characterized by `data-raw/m76-coverage-sweep.R`. See `cairn/references/burch2011.md`, `ohyama2025.md`, and `classical-oneway-comparison.md`.
