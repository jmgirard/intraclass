...
- **Decision:** exported as the balanced one-way `ci_method = "npbootstrap"`; percentile & BCa NO-GO (D-006). Point estimate is the engine REML point for both estimands (BC5); the ANOVA-MoM ρ̂ is interval machinery only.
- **Provenance:** ukoumunne2003 Table I (external) + reproducible in-suite fixtures; regenerate with `data-raw/m75-npbootstrap-parity-oracle.R` (parity) and `data-raw/m75-npbootstrap-coverage.R` (sweep). See D-006 (GO/NO-GO), D-010/RR02 (exported-API scope), and `cairn/references/ukoumunne2003.md`.
