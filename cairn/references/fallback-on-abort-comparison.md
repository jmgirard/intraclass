...
`data-raw/m111-fallback-verdict.R`, which applies the frozen rules
mechanically — every figure below re-derives from the committed fixture by
re-running that script (rule verdicts and conditional coverage/tails from the
ledger it writes; abort rates and F4 widths from the fixture's summary table
it prints).

**F1 — interval always: PASS, both arms** (0 aborted rows among the 256,000
classical-leg computations — 128,000 reps × 2 legs; asserted on the raw
per-leg abort indicator, corrected at review from a vacuous composite-NA
check).
...
2–9% at ρ = 0.60, and the 10×5 design aborts on 6–12% at ρ = 0.30 (t5/chisq1).
...
interval computed from such a sample sits below the truth — every conditional
miss at every applicable cell is upper-tail (cond. lower-tail miss 0.000
everywhere, both arms; upper-tail miss up to 0.928 Burch / 1.000 SEARLE at
the failing cells). The collapse tracks the design's information, not ρ
alone: at the wide 10×2 design even ρ = 0.30 abort samples get intervals wide
enough to cover (Burch cond. 1.000 at all four (0.30, 10, 2, ·) cells), while
informative designs — (10, 5) at ρ = 0.30, (50, 5) at ρ = 0.10, and the
ρ = 0.60 10×2 chisq1 cell — produce narrow fallback intervals that miss.
...
(failing-cell composite coverage 0.816–0.929 SEARLE / 0.838–0.920 Burch) and
degrades badly on skewed data at large ρ — at (0.60, k≥30, 5, chisq1) the
composite ≈ MC alone covers **0.673–0.676** with 0 aborts, an incumbent
defect the fallback never touches. Eight cells per arm over-cover past
F2's ~0.97 descriptive band (to 0.9945, near-zero-abort cells — MC-leg
behavior).

**F5 — tail symmetry: SEARLE fails 45/64, Burch 48/64**, dominated by the MC
leg's lower-tail miss asymmetry (lo_miss up to 0.16 at the n=2 cells, where
hi_miss is ~0.02 at ρ ≤ 0.10 but reaches 0.093 at (0.60, 10, 2, chisq1);
the fallback legs' all-upper-tail conditional misses add asymmetry of the
other kind).
...
binding rules at every applicable cell. The decisive F3 failures are
structural (conditional coverage 0.00–0.49 against the 0.93 floor); on F2 the
margin is mixed — 4 SEARLE and 5 Burch cells fail within 0.005 of the floor,
the rest by more (the "1 SEARLE / 4 Burch" figures quoted here before review
were F2 *near-passes*, the tie-break statistic, corrected at review):

- **D1 — the fallback question dies on selection, not on the fallback's own
  calibration.** Conditional on an abort at an informative design — one
  whose classical interval is narrow, per the F3 pattern above — no assessed
  interval covers: the abort selects degenerate samples, and the better the
  design, the worse the conditional miss (all misses upper-tail). A GO would
  hand users a confident-looking interval in exactly the reps where the data
  are least representative.
- **D2 — the status-quo abort is vindicated at the informative-design abort
  cells** (it refuses to report where no assessed method covers
  conditionally; at the wide 10×2 cells Burch does cover, but no shipped
  diagnostic can tell a user which case their abort is) and the boundary
  case is already served by the opt-in `ci_method` values (D-012/D-013): a
  user who hits the abort is told which opt-in applies (M93/D-018), which
  keeps the choice — and the interpretation burden — with the user.
- **D3 — reopening evidence class:** a fallback construction that models the
  ...
  restricted to the low-information abort cells where Burch's conditional
  coverage measured 1.000 — 28/29 ρ ≤ 0.10 cells plus all four
  (0.30, 10, 2, ·) cells; the (0.10, 50, 5, chisq1) exception shows the
  boundary is informational, not a ρ threshold. Composite-vs-abort preference
