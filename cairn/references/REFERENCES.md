# References — split

**Provenance.** Ingested 2026-07-12 by the `/cairn-init` migration from the pre-migration single references page, reduced to this pointer stub by M63 (D-007).
Pagination: —.
Extraction: first-hand record — a pointer stub with nothing to re-verify against — observed 2026-07-18.

Split by M63 (D-007, 2026-07-18): oracle registry → [`ORACLES.md`](ORACLES.md);
bibliography → [`BIBLIOGRAPHY.md`](BIBLIOGRAPHY.md); per-source extractions →
`<citekey>.md` notes via [`INDEX.md`](INDEX.md). Kept so links from the entombed
`cairn/legacy/` and `data-raw/reviews/` still resolve.
