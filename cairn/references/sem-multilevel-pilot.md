...
**Provenance.** Ingested 2026-07-16 by M53 from its own two-level lavaan pilot runs, composing ten Hove, Jorgensen & van der Ark (2022) (the estimand) and Jorgensen (2021) (the SEM-GT mean-structure device) **as papers** — the `tenhove2022.md` and `jorgensen2021.md` source notes did not exist until M64 authored them on 2026-07-18, so this page is not downstream of them.
Pagination: —.
Extraction: derived — no external source of its own, only as current as the papers it composes, none re-read since 2026-07-16 — observed 2026-07-18.
