...
- [ ] T1 — Acquire + note the Burch REML ICC-CI source (maintainer supplies the
      PDF to `cairn/references/sources/`; write its `BIBLIOGRAPHY.md` + source
      note extracting the interval construction). Extend `ukoumunne2003.md` /
      confirm `mcgraw1996.md` Table 7 for the SEARLE-F leg. **Gate: the Burch
      leg cannot start until the source is on the shelf** (IP1/#1 — no method
      from memory). `(RB tripwire: no-oracle)` if no reference value is extractable.
- [ ] T2 — Prototype SEARLE exact-F CI in `data-raw/`; validate against
      `mcgraw1996` Table 7 + `ukoumunne2003` exact-F (AC1).
- [ ] T3 — Prototype Burch REML CI per the supplied source; validate against its
      reference value (AC1). Gated on T1.
