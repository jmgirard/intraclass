...
- 2026-08-27: T2 done. Two bullets added to *What ships* after the `icc()` bullet: one for the imbalanced/incomplete/multilevel design support and cluster-level reporting, one for within-cell replicates and `occasions`. Bullet bytes 437 and 327, both under M142's 500 cap; no pinned region touched. (Corrected at the 2026-08-28 merge gate: the line first recorded 393 for the second bullet, which no measurement of the committed text yields; the merge-gate rewrite moved the pair to 481 and 413.)
