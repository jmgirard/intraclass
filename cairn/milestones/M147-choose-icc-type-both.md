...
- [x] T3. `build_icc_call()` omits `type =` when `type == "both"`, the shape
...
- [x] T4. Roxygen `@param type` states `"both"` in the shape `@param unit`
...
- [x] T5. Gate: `air format .`, `devtools::document()` no-diff,
...
- 2026-08-28: T5 gate complete. `devtools::check()` raw `Status: OK` (line 35 of the run's output), reported as 0 errors, 0 warnings, 0 notes over 14m 23.8s on intraclass 0.1.0. Status -> review.
- 2026-08-28: T5 gate, checks other than `devtools::check()`: `air format --check .` clean; `devtools::document()` produced no diff; `devtools::test()` `[ FAIL 0 | WARN 3 | SKIP 2 | PASS 9465 ]` -- the same WARN 3 the ROADMAP candidate row records on the default branch (two fitting diagnostics plus the fixed-rater advisory from an unwrapped `icc()` call at `test-icc-brms.R:2425`), unchanged by this branch. All six `data-raw/` checkers pass `--self-test` and live: `check-mpl-doc-claims.py` (60 candidates, 12 settled), `check-oracle-registry.py` (64 asserted, 0 gaps), `check-record-claims.py` (7 claims re-derived), `check-reference-observations.py` (85 observations, 0 falsified), `check-abort-remedy-verdicts.R` (52 cells, 0 broken promises), `check-checkpoint-sites.R` (5 sites routed; 130 planted mutations all detected).
- 2026-08-28: T4 done. AC4's `grep -rn 'choose_icc' R/ man/ vignettes/ README.Rmd NEWS.md` returned 36 hits after the sweep, across `R/choose-icc.R` (17), `R/abort.R` (1), `man/choose_icc.Rd` (10), `vignettes/choosing-an-icc.Rmd` (3), `vignettes/multilevel-designs.Rmd` (3), `README.Rmd` (3), `NEWS.md` (1). Disposition: five stated or implied a single recommended coefficient and were reworded to "the coefficient or coefficients" -- `vignettes/choosing-an-icc.Rmd:255`, `vignettes/multilevel-designs.Rmd:339`, `README.Rmd:98`, `NEWS.md:35`, and the `R/choose-icc.R` `build_icc_call()` comment already corrected in T3. The remaining 31 are the function's own name in code, `@rdname` tags, `\name`/`\alias`/usage lines, example calls, and prose making no claim about how many coefficients come back or which type values are accepted; none needed an edit. Two surfaces the grep cannot reach (their lines do not contain the string `choose_icc`) were swept under the Scope clause instead: the roxygen `@param type` and its generated `man/choose_icc.Rd:28` entry, both of which now name `"both"`. NEWS, the *Choosing an ICC* article and README also gained a sentence naming the value; `README.md` re-knitted.
- 2026-08-28: T3 done. `build_icc_call()` drops `type =` when both are wanted and its stale "resolves to ONE coefficient" comment is corrected; the rationale gains its both-type sentence and `recommendation_notes()` gains a `type` argument so it can say the omission is deliberate. `format.icc_recommendation()` expands `"both"` before calling `icc_design_phrase()`, which takes a vector and would otherwise `switch()` on an unrecognised string -- the header now reads "two-way random, absolute agreement & consistency", matching a fit reporting both. The interactive-walkthrough snapshot re-recorded for the third type choice (diff: the reworded question line plus a "3. Both" option). `test-choose-icc.R` green with NOT_CRAN set: 108 pass, 0 fail, 0 skip.
