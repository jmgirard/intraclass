...
- [ ] AC1 (GP7): a committed text fixture under `tests/testthat/fixtures/` carries
...
- [ ] AC2: a test in `test-ci-mpl.R` asserts the shipped `kappa_m_table` matches that
      ...
      a changed one.
- [ ] AC3: the fixture is built from the calibration fixtures, never from
...
- [ ] AC4 (the M92 P6-1 lesson made mechanical): the pin is mutation-verified, not
...
- [ ] AC5: the mutation record includes a perturbation confined to cells NOT covered by
...
- [ ] AC6: `devtools::test()`, `devtools::check()`, `lintr::lint_package()` and
