...
- [x] T11 (pass-9 F1, 91; thrash trigger 2): the AC5 leak guard asserts a
      ...
      Mutation: a `round(., 3)` leak in EACH of the three rendered bullet forms must
      red it, including the singular one-way lead that is pass-9 F1's own.
...
- 2026-07-26: T2 — the hint is now ADMISSIBILITY (would `icc()` accept this string on this design) plus USABILITY (run it and look). `boundary_data_degenerate()`, `boundary_sb_safe()` and `boundary_classical_sb_ok()` are deleted, as are the `n_s`/`n_r`/`complete`/`degenerate`/`sb_ok` arguments; the builder now takes the `df`, `estimands` and `conf_level` the dispatch already holds, plus `n0` as a support constant. `mpl_kappa_available()` went too — added at pass 1 for the hint alone, and dead once the lookup's own abort settles the grid. End-to-end: one-way default names both methods, `unit = 6` still names both at 30 subjects, `unit = 15` falls SILENT with no pole rule in the code, two-way on-grid names mpl, off-grid and fixed-rater name nothing — all with class `intraclass_singular_fit` and both generic remedies intact. Message reworded to "run on your data" per the gate. The hint's own tests moved with it (a hand-written predicate list can no longer stand in for data, which is pass-2 F4 made structurally impossible), and `bh_usable()` gained the in-support clause so the suite's predicate matches AC3's. Suite FAIL 0 / PASS 4668 / SKIP 23 at `NOT_CRAN=true CI=true`; `lintr` no lints; `air format --check` clean.

- 2026-07-26: T3 — AC4's two anti-drift tests. The equivalence half compares each reducer's endpoints against `tidy(icc(ci_method = ))` at `tolerance = 0` across 21 cells, and the admissibility half asserts each row's methods are accepted by `icc()`'s fence while the excluded designs really are refused `intraclass_unsupported`. Mutation-verified, and the first two mutations both FAILED to red it, which is the finding: perturbing `searle_ci` moves both sides at once (icc() calls the same reducer), and a `pmin(1, .)` clamp in the reporting path was invisible because every cell in the grid returned in-support endpoints. Fixed by adding `bh_pole_oneway()` — 2×2, `unit = 15`, `conf_level = 0.80`, where searle reports [1.153869, 1.164311] — after which the clamp mutation reds the test. Pinned with `out_of_support_seen` plus exact `checked`/`compared` counts, so a grid that loses that cell reds instead of quietly weakening. Suite FAIL 0 / PASS 4763.

- 2026-07-26: T4 — the sweep now asserts `named == usable` among the methods a design ADMITS, which carries both AC3 halves in one assertion and changes what a silent cell means: it no longer passes for free, it asserts that nothing admissible would have worked. That is why pass-5 F2's two empty `unit` cells are kept rather than deleted — under the converse they check something — and why the tautological `expect_gte(checked, 0L)` is simply gone. Every declared cell now asserts it fired the abort at least once, and each sweep asserts a method was named somewhere, so universal silence cannot satisfy it. Cells added: an uncalibrated `conf_level` and an explicit-consistency design; off-grid `mpl` is now carried by the converse half rather than by a fence. Mutation-verified four ways: naming searle/burch unconditionally reds 10, dropping the in-support clause reds 3, dropping the mpl verification reds 10, and neutering one cell so it never aborts reds 3 at the non-vacuity assertion. Suite FAIL 0 / PASS 4872 / SKIP 23.
...
- 2026-07-27: T11 — the AC5 leak guard no longer enumerates branches. It sweeps a 6-cell grid over the hint's INPUTS (design, balance, `unit`, `type`), records every bullet `boundary_method_hint()` returns and every interval `boundary_interval_usable()` inspects through `local_mocked_bindings()`, and asserts the invariant over whatever comes back: a bullet carries no digit, raw and rendered. Rendering goes through `cli::format_message()`, the formatter `cli_abort()` itself applies — `format_inline()` leaves cli's `\\` continuations in the string and would have made the end-to-end comparison untestable — and each cell pins its own bullets as substrings of the real abort it fired, which is also what keeps the renderer honest. Non-vacuity is a property of the OUTPUT rather than of a list: the grid must see a bullet naming one method and one naming two, so the singular and plural leads are both read, plus a pin on the distinct-rendering count. Mutation-verified seven ways against the AC5 test's own baseline (FAIL 0 / PASS 225): `round(., 3)` leaked into the mpl bullet reds 4, into the plural lead 3, and into the SINGULAR lead — pass-9 F1's own, which left the predecessor at FAIL 0 / PASS 720 — 3; a grid drifting off the splitting cell reds 1; a REACHABLE new rendering reds 1; a stub renderer reds 4. The one mutation that does not red is an UNREACHABLE new literal (a searle-only lead), measured unreachable across 11 `unit` values × 2 data shapes × 6 seeds, where only the burch-only and searle+burch forms occur — nothing renders it, so no user can be shown it either. Suite FAIL 0 / PASS 5105 / SKIP 23 at `NOT_CRAN=true CI=true`; `lintr` no lints; `air format --check` clean.
- 2026-07-27: recorded so the readings are not double-counted — the FIRST mutation round was void and was redone. Restoring a mutated fixture with `git checkout -- tests/...` reverted T11 itself, which was still uncommitted, so four of that round's readings were taken against the superseded T10 test; only the three `R/`-only readings were valid. The redo commits the guard first and restores from a scratchpad copy. Every figure in the T11 line above is from the redo.

- 2026-07-27: guard-doctrine §8 certification round — a fresh-context [O] reader that authored none of this returned 4 discrepancies in the description layer, 2 substantive; each fixed, none argued down. **D1** (supersedes the mutation figure in the T11 line above): the non-vacuity comment and three records claimed `1L %in% named_counts` pinned that the SINGULAR one-way lead was read, and it did not — the `mpl` bullet names one method too, so the pair passed with that lead never rendered, which is the pass-9 shape reproduced inside its own replacement. Fixed in the CODE, not the wording: the counts are now taken over bullets naming only classical methods, read off the output's own method names, and the grid-drift mutation reds 2 where it used to red 1. **D2**: AC5's first clause was credited to a test ending in `skip_if_not(found, ...)`, which goes GREEN on the one violation it forbids — `icc()` returning an interval instead of aborting — and whose `expect_false(inherits(got, "icc"))` was a tautology inside the condition branch; the sweep now fails rather than skips and inspects the returned value outside the branch, verified by a new mutation (healthy fixture → FAIL 3 / SKIP 0, where the old form skipped green). **D3**: "the formatter `cli_abort()` itself applies" is false — `cli_abort()` runs `format_inline()` and rlang wraps afterwards, so `format_message()` agrees by the end-to-end substring check rather than by construction; the comment now says that. **D4**: "three message literals" is five literals rendering three bullet forms; corrected in the task and the comment. Two further observations adopted into the file rather than implied away: the site-A arm is reachable but unexercised (every grid cell lands on site B), and the positive controls control `num_tokens()`, not `render()`. Post-fix: M93 file FAIL 0 / PASS 893 / SKIP 0, the AC5 guard 226 assertions and the contract test 3; leaks into the mpl bullet / plural lead / singular lead still red 4 / 3 / 3.

- 2026-07-27: §8 round 2 — D1-D4 all certified CLOSED, D1 in the code rather than the wording: a counterexample cannot be built, because a classical bullet's method count IS `length(named)` and the lead is chosen on that same value, so a 1 in `oneway_counts` implies the singular lead was rendered. Two NEW discrepancies, both in comments the fix commit itself added, both fixed. (i) A site-A frequency, "~1 in 40 at these geometries", which is T1's rate over ITS OWN six boundary geometries and not this grid's — removed rather than restated, since no measurement of this grid's site-A rate exists here and inventing one is what #4 forbids; the comment now says only that every cell lands on site B, which is measured. (ii) "Both predecessors … blind to one", which contradicts the file's own "Three predecessors" header and, with the `numeric` qualifier added in the same commit, describes pass 7 alone — now split into the numeric-rendering case (pass 7) and the bullet-form case (passes 8-9). Suite FAIL 0 / PASS 5106 / SKIP 23 at `NOT_CRAN=true CI=true`; M93 file FAIL 0 / PASS 893 / SKIP 0; `air format --check` clean.
