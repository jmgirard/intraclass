...
random-only); nested fixed lavaan (no SEM parameterization) → still refused. The bootstrap for this fixed cell reuses
M56's factory only if M56 has merged; otherwise MC-only, bootstrap deferred to a
follow-on note.
...
- [ ] AC2: the fixed **cluster**-level ICC(c,·) agrees with glmmTMB fixed cluster
      (M37 / ADR-047) and equals the random-rater cluster-level ICC on balanced
      data (the M37 exact reduction: θ²_r == σ²_r, σ²_cr unbiased under fixing).
