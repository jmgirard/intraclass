...
- 2026-08-05: CI on 3f221dd -- check-references, lint, format-check and pkgdown green; ubuntu, windows and coverage still running at the blocking-wait timeout. Not presenting the merge gate until the Windows job reports: it is the only surface that exercises the exact-zero branch this pass exists to fix, and approving on the four fast jobs alone would repeat the mistake that caused the pass-2 return.
