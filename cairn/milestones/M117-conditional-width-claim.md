...
- AC2 — **FAIL (found by the independent review, not the evidence runs).** The suite-level evidence was green — installed run 0 failed / 1177 passed / 0 skipped; 18 prose mutations all refused, control clean; claimed-classes header present — but the [O] lens found and this session reproduced F1: `width_loose_ratios()` discards the `ok` verdicts of the claims it consumes, and the canonical-claim `ok` checkers run only inside width neighbourhoods, so a FALSE figure in canonical form placed elsewhere on a reported site is consumed unchecked (a patched article with "0.8123 at a true ICC of 0.3" — the true median is 0.9475 — passes the whole suite and `prose_scan`). Inside AC2's stated domain. The reproduction also shows AC2's letter over-reaches on the two bare level shapes: surface-wide `ok` enforcement of `k_level` reds on the article's legitimate "100 subjects" sentence, so the fix pairs a scoped code change (surface-wide `ok` for the value-checkable shapes, the bare level shapes staying neighbourhood-scoped as consumption aids) with a gated AC2 wording amendment stating that two-tier scope.
...
CI green on the review head `e47c18d` (REST poll, all nine checks). [O] diff-bug: 11 findings, F1 independently reproduced by this session before scoring; its verified-clean list re-derives every stated doc figure from the fixture. [S] blame-history: 3 candidates, 2 checked-clean (the mpl re-key follows M105; D-012 Amendment 1's counts preserved). [S] prior-review: one candidate (pass-3 PR-1 carried forward); GitHub comment probe empty. Scored by a fresh [S] scorer holding the diff and the amended plan: 2 findings at or above 80.

**Actioned (≥ 80), verbatim:**

- F1 (92) — "`width_loose_ratios()` discards ok verdicts; canonical claims are ok-checked only inside width neighbourhoods, so a FALSE figure in canonical form outside a neighbourhood passes the suite (verified: patched article, 0 failed/1177 passed). AC2's text scopes checking to 'every site AC5's sweep reports' (= files), so this is inside AC2's stated domain." → **drives the return** (floor: AC2 failing inside its named procedure's domain).
- F4 (80) — "`prose_scan` mirrors only part of the suite (omits rater forbidden/licensed rules, table-row completeness, smaller-grid-stated completeness, per-surface run floors); no rater-class mutation exists among the 18, so the T18 work-log claim 'every review-return defeat encoded' is overstated (the rater defeat cannot red in the harness)." → folded into whatever round is convened (add the rater scan + mutation; correct the work-log overstatement by a superseding line).

**Logged, below the action bar (15):** F6 (76, the harness class list names 15 of 18 mutations — the three AC9 restorations sit outside the block unlabelled); F3 (68, the `smaller_grid_pooled` mutation states 0.9440 as the smaller grid's median — the true pooled median is 0.9430, an unmeasured-value probe the work log's own rule forbids); B3 (68, the guard-scope narrowing is recorded only in the work log, not as a DECISIONS.md entry, unlike D-020 Amdt 2 / D-021 / D-029); F2 (58, orphan refusal keys on the ratio band, so an out-of-band figure under an unknown header escapes — band boundary undeclared in the header); F5 (58, `level_count` is unreachable dead code, consumed first by `k_level` — E1 recurrence, fails conservatively); F8 (55, DESCRIPTION/NAMESPACE roxygen2 8.1.0 version churn — environment artifact, flips back under an older roxygen2); PR1 (48, boundary-hint restates the template clauses in a `#` comment its sibling declines to — stylistic asymmetry, no functional consequence, both unswept under `R CMD check`); F7 (45, patch anchors assert ≥1 hit not uniqueness); B2 (32, a >60-char genuine table cell would fail closed as loose prose — none exists); F9/F10 (20 each, prose style); F11 (20, observation — malformed-row coverage rides partly on a collateral orphan but is genuine); B1 (10, the promise narrowing is the maintainer's own gate decision); B4/B5 (10 each, checked clean).
