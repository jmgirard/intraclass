...
- [x] AC1 A committed stdlib-only `data-raw/check-record-claims.py` reads
      ...
      checker supports is listed in the docstring and runs with `python3` and the
      runner's POSIX utilities alone — the `check-references` job has no R and no
...
- 2026-08-03: pass-5 fix pass, at the maintainer's direction to fix the six pass-4 actioned findings in place rather than route through `/milestone-plan` as thrash trigger (a) prescribes — a logged user override, not a rule read differently. O4: the docstring now states BOTH channels AC2 requires it and D-020 Amendment 2 to state jointly; `grep -n "\.git" data-raw/check-record-claims.py`, the command that measured the failure, returns four matches at this commit against none before. O7/O6: the checker's `D-045` and `IP4` citations named ids this repo does not define (`cairn/DECISIONS.md` holds D-001…D-021, `cairn/DESIGN.md` IP1–IP3); the comment now states the current-knowledge/history split in its own words and qualifies both as the plugin's, matching this repo's existing `cairn D-024` convention. O1/B2: `lint.yaml`'s "history-free by construction" is replaced by what is actually true — every `git` command a row spells is refused, which is a rule over the cell and not a sandbox.
