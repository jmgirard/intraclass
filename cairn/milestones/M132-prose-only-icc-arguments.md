...
- 2026-08-22: gate follow-ups — two candidate ROADMAP rows added (the `?icc` `@param design` blurb disagreeing with the widened article; the `school` fixture rebuilt verbatim five times in `test-vignette-claims.R`). Adding them put `ROADMAP.md` at 25,215 bytes, over the 24,000-byte budget, so the three widest candidate rows were compressed in the same commit (per-class reachability proof, checkpoint blind spots, merDeriv bootstrap caller) and two clauses trimmed from the new rows: 23,952 bytes, 54 lines. Detail moved out of the compressed rows already lives in the archive files and git text each cites.
