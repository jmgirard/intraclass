...
- 2026-09-10 consistency gate: `cairn_validate.py` all checks passed (16 PASS, 7 OK advisories, `release window` not fired); no principle changed, `cairn_impact` skipped; `devtools::document()` no diff; README.Rmd/README.md untouched by the diff; `pkgdown::check_pkgdown()` no problems; NEWS: none owed (tests-only, no user-visible change); no new top-level files; `devtools::check()` on the branch at a213038: 0 errors, 0 warnings, 0 notes (23m55s). Gate passed.
- 2026-09-10 review fan-out (fresh-context, ref-based git): [S] prior-PR-comments — no regression; the one prior finding on these files (M141 review [O], the brace defect) is what this diff resolves; PR-comment probe returned empty. [S] blame-history — no conflict with history; frame sentences verified verbatim in `R/engine-lavaan.R` and `R/engine-glmmtmb.R`, `intraclass_fixed_raters` in `R/abort.R`; re-ran three files green; one observation: LESSONS 2026-07-27 (M93) says a test wanting user-seen text calls `format_message()` itself — it found no `\\`-continuation message in `R/`, so nothing lost by dropping it. [O] diff-bug — six ranked findings, triaged at the gate below.
- 2026-09-10 [O] finding 1: the two frame-sentence assertions turn a platform-dependent engine warning into a hard failure on CI-run tests (`test-icc-lavaan-multilevel.R:402`, `test-icc-type-vector.R:286`); a quiet convergence on another platform or engine version fails the wrapper while the abort/message still fires.
- 2026-09-10 [O] finding 2: `expect_warning()` consumes one warning; a second identical one escapes to the suite's WARN line rather than failing the test, so "asserted at its site" is enforced only by reading that line (each site currently emits exactly one, verified).
- 2026-09-10 [O] finding 3: the brms-file site (`test-icc-brms.R:2431`) runs under pre-existing `skip_on_ci()`, so CI never exercises that `expect_warning()`; the glmmTMB call passes with exactly one warning locally.
- 2026-09-10 [O] finding 4: the brace test (`test-n-o-disposition-grid.R:341`) relies on the package not opting into rlang's cli formatting; were `.rlang_use_cli_format` added, `rlang::abort("count {x} …")` itself would error, and the test would error rather than fail — `use_cli_format = FALSE` on the abort would decouple it.
- 2026-09-10 [O] finding 5: the new `test_that()` block sits between two helper definitions in the `# ---- running one case` section rather than a test section.
- 2026-09-10 [O] finding 6: the renderer comment "never pass it back through cli" overstates — `cli::ansi_strip()` is still called (no interpolation, so harmless).
- 2026-09-10 triage (maintainer, at the gate): finding 1 rejected — an intentional plan-gate choice (assert, never suppress; falsification condition recorded), and the Ubuntu R 4.5.0 CI check passing on the code head shows the warnings fire off this machine; finding 2 rejected — the suite's WARN line is the instrument AC3 names, a second identical warning being new package behavior for its own row; finding 3 rejected — pre-existing `skip_on_ci()` the diff did not introduce; finding 4 fixed now — the brace test's abort passes `use_cli_format = FALSE` (`test-n-o-disposition-grid.R:345`); finding 5 rejected — style, the test sits beside the helper it pins; finding 6 fixed now — the renderer comment names `cli::format_message()` as the call avoided. [S] blame-history M93-LESSONS observation: noted, no action. Post-fix: grid file green, 0 lints, `air format` clean.
