...
- 2026-08-02: pass-5 gate disposition, maintainer's choice: BLOCKED on the claims-ledger checker (the ROADMAP candidate row carrying RR04's BC2/BC3/BC6), rather than a sixth hand-fix pass, a merge with the findings recorded, or a drop. The blocker is named because pass 5's failure is that mode exactly: AC4 was certified by a hand-written grep whose pattern could not match the violation it was certifying. M100 resumes once a committed (claim, command, expected output) ledger and its CI checker exist to re-derive its figures and rule claims mechanically. The five actioned findings stay open and are the first work of that resumed pass; no work proceeds on this branch meanwhile.
...
- 2026-08-02: the blocker named at the pass-5 disposition is now M102 ("A registered claims ledger, and the CI checker that re-derives it"), planned this date. The graduation and this milestone's `Depends on: M102` land on the default branch with M102's plan commit, not here, so THIS branch's ROADMAP still carries the superseded candidate row and `Depends on: —` until the resumed pass merges the default branch in (`git merge --no-edit $(git rev-parse origin/main)`, per the M71/M93 lesson) — do that first. M102 does NOT register this milestone's figures — `data-raw/abort-remedy-sweep.tsv`, the enumerator, the sites TSV and the enumeration exist only on this branch, so a branch cut from `main` cannot settle a row against them (`git ls-tree --name-only origin/main data-raw/` lists none of the four). Registering them, and wiring `enumerate-ci-method-remedies.py` into `lint.yaml` (RR04 BC6's first half, likewise impossible while the script is branch-only), are the resumed pass's work alongside the five open findings.
