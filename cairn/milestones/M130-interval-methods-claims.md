...
### Triage and the fix-now work (2026-08-22, at the approval gate)

All 13 findings were put to the maintainer at the gate with their text verbatim.
Dispositions: **P1, P2, P3, P4, P6, P7, P8 fixed now** on this branch before the
approval marker; **P5 and P10 rejected** under the out-of-scope taxonomy (P5 is
not the F6 shape — a bound is the natural form of a "sit close" claim; P10's 0.93
floor is the repo's pre-existing convention in `test-doc-skew-caveat.R`, on a line
this diff does not touch); **P9, P11, P13 noted, no action** — none is a criterion
failure and each is recorded above; **P12 resolved** by this block's own pass.

What the fix-now commit does, by finding:

- **P1** — `R/icc.R:396-402` now states both unit outcomes at zero
  between-subject variance, matching the vignette and the NEWS bullet that
  describes it; `man/icc.Rd` regenerated by `devtools::document()`. This is the
  one change that adds `R/` and `man/` to a diff the plan scoped as
  vignette-and-tests; the maintainer chose it at the gate over narrowing the
  NEWS sentence, because it removes a false claim a user can read today rather
  than only stopping the changelog from asserting a correction that had not
  happened.
- **P2** — `NEWS.md:417-420` now names the claim the released article actually
  made ("the upper bounds coincide", unqualified) rather than this branch's
  intermediate draft wording, and adds the two-sidedness the tests assert.
- **P3** — the committed-fixture cross-check moved out of the `data-raw/`-fenced
  third-grid test into its own unfenced block
  (`tests/testthat/test-vignette-claims.R:1812`), so article lines 207-212 keep
  one assertion inside a built package; the sibling comment at `:1703-1707`,
  which claimed the leg "does ship" while it did not, now points at that block.
- **P4** — the matched-fit comment now says `tolerance` is relative, with the
  measured 0.0844 against the plant's 0.704 recorded beside it.
- **P6** — the two single-level lavaan rater-mode cells and the two-level cell
  gained the file's own `expect_gt(nrow(td), 0L)` anti-vacuity idiom.
- **P7** — `expect_gt(gap_restricted, 0)` became a floor at `1e-4`, decades below
  the measured 0.005987, the same realization-vs-property repair O7 took.
- **P8** — the walker's comment no longer calls its `^r[a-z]+$` name filter
  exhaustive; the two assertions resting on it now say they fence `r*`-spelled
  draws and no wider.

Re-verified after the fixes: `NOT_CRAN=true CI=true devtools::test()` 0 failures,
0 errors, **8500 passing**, 25 skipped (8497 before; the three added assertions),
the same 2 pre-existing warnings outside this diff. `air format --check .` clean;
`lintr::lint_package()` 0 lints; `devtools::document()` idempotent (no diff beyond
