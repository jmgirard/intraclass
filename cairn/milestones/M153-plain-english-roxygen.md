...
- [ ] T4: R6 hunk audit per `prose-style.md` step 5 over every hunk of T1 to T3; repair widenings in place; run `devtools::document()`, the ruler, `check-mpl-doc-claims.py`, and the AC2/AC3 greps; record each AC1-exempt sentence with its word count and its outside-text count, and any change to a pinned string, in the work log; add the NEWS bullet.
