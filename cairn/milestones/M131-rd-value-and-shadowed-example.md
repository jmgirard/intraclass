...
- AC3 — `_R_CHECK_CRAN_INCOMING_REMOTE_=false R CMD check --as-cran --no-manual <tarball>` on `R CMD build` tarballs, back-to-back under R version 4.6.1 (2026-06-24): branch tip 426fa02 `Status: 1 NOTE`, merge-base 53f77a1 `Status: 1 NOTE`; sole NOTE heading on both `checking CRAN incoming feasibility`, no WARNING heading on either, so the branch adds no heading the base lacks. No `--as-cran` check heading mentions `\value` — observed 2026-08-22 on R 4.6.1. Re-run identically at the post-fix tip c0c53d9 after the review fix-now round: branch `Status: 1 NOTE`, base `Status: 1 NOTE`, same sole heading, no WARNING, no `\value` heading.
