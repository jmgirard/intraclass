...
cells_ho <- do.call(
  rbind,
  lapply(split(ho, ho$cell), function(g) {
    na <- !g$mc_aborted
    data.frame(
      cell = g$cell[1],
...
if (length(passers) == 0L) {
  ...
  message(sprintf(
    ...
    ledger$form[win],
    format(ledger$c_k[win]),
    format(ledger$c_g[win]),
    ledger$max_protected_fire[win],
    ledger$min_targeted_fire[win],
    length(passers)
  ))
...
write.table(
  led_out,
  "data-raw/m114-warn-trigger-candidates.tsv",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
win_out <- winner_tbl
for (cn in c("fire_rate", "cov", "ab")) {
  win_out[[cn]] <- fmt4(win_out[[cn]])
}
write.table(
  win_out,
  "data-raw/m114-warn-trigger-verdict.tsv",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
