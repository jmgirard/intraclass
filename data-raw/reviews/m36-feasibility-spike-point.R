...
theta2r_nested_ragged <- function(beta, vbeta, cluster_of) {
  ...
  per <- vapply(
    ...
    function(ix) {
      kc <- length(ix)
      if (kc < 2L) {
        return(c(raw = NA_real_, bias = NA_real_))
      }
...
run_bias <- function(
  ...
  est <- t(vapply(
    ...
    function(i) {
      ...
      l <- tryCatch(fit_and_estimate(d, "lme4"), error = function(e) {
        list(icc = NA, theta2 = NA)
      })
      c(
        icc = g$icc,
        theta2 = g$theta2,
  ...
  cat(sprintf("\n== %s ==\n", label))
  cat(sprintf(
    "  truth: theta2_{r:c}=%.4f  sigma2_s=%.3f  sigma2_e=%.3f  ICC(A,1)=%.4f\n",
    th_true,
    s2s,
    s2e,
    icc_true
  ...
  cat(sprintf(
    "  est  : theta2=%.4f (bias %+.4f)  sigma2_s=%.3f  sigma2_e=%.3f\n",
    mean(est[, "theta2"]),
    mean(est[, "theta2"]) - th_true,
    mean(est[, "s2s"]),
    mean(est[, "s2e"])
  ))
  cat(sprintf(
    "  ICC  : mean=%.4f  bias=%+.4f  (rel %+.1f%%)  sd=%.4f  R=%d\n",
    mean(est[, "icc"]),
    mean(est[, "icc"]) - icc_true,
    100 * (mean(est[, "icc"]) - icc_true) / icc_true,
    sd(est[, "icc"]),
    nrow(est)
  ))
  cat(sprintf(
    "  cross-engine |dICC| max=%.2e mean theta2 diff=%.2e\n",
...
run_bias(
  "A: equal k_c=4, 25% cells missing",
  Nc = 8,
  kc_vec = rep(4, 8),
  ns = 12,
  s2s = 1.0,
...
gammaB <- rnorm(8, 0, 1)
run_bias(
  sprintf("B: unequal k_c=%s, 20%% cells missing", paste(kcB, collapse = ",")),
  Nc = 8,
  kc_vec = kcB,
  ns = 12,
  s2s = 1.0,
