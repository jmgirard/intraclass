...
sim_one <- function(
    ...
    for (s in seq_len(ns)) {
      ...
      for (r in seq_len(kc_vec[c])) {
        ...
        rows[[length(rows) + 1]] <- data.frame(
          cluster = c,
          subject = paste0("c", c, "s", s),
          rater = paste0("c", c, "r", r),
  ...
  d$cs <- factor(paste0(d$cluster, ":", d$subject))
  d[d$cs %in% names(which(table(d$cs) >= 2)), , drop = FALSE]
}
true_theta2 <- function(rl) {
  mean(vapply(
    rl,
    function(m) sum((m - mean(m))^2) / (length(m) - 1),
    numeric(1)
  ))
}
...
estimate_with_ci <- function(d, ndraw = 2000) {
  ...
  beta <- fixef(f)$cond
  k <- length(beta)
  V <- as.matrix(vcov(f, full = TRUE))
  nm <- colnames(V)
  b_ix <- seq_len(k)
  disp_ix <- which(nm == "disp~(Intercept)")
  th_ix <- grep("^theta", nm)
  vb <- V[b_ix, b_ix, drop = FALSE]
  cluster_of <- sub("^rater(c[0-9]+)r[0-9]+$", "\\1", names(beta))
  idx <- split(b_ix, cluster_of)
  ctr <- lapply(idx, function(ix) {
    kc <- length(ix)
    diag(kc) - matrix(1 / kc, kc, kc)
  })
  kc <- lengths(idx)
  bias <- Map(
    function(ix, C, kk) sum(diag(C %*% vb[ix, ix, drop = FALSE])) / (kk - 1),
    idx,
    ctr,
    kc
  )
  ...
  raw <- Map(
    function(ix, C, kk) as.numeric(t(beta[ix]) %*% C %*% beta[ix]) / (kk - 1),
    idx,
    ctr,
    kc
  )
  s2s <- exp(2 * getME(f, "theta"))
  s2e <- sigma(f)^2
  th_pt <- max(0, mean(unlist(raw) - unlist(bias)))
  icc_pt <- s2s / (s2s + th_pt + s2e)
  # draws
  L <- t(chol(V))
  par <- c(beta, V_disp = NA)
  mu <- c(beta, log(sigma(f)), getME(f, "theta"))
  ...
  s2s_d <- exp(2 * draws[th_ix, ])
  s2e_d <- exp(2 * draws[disp_ix, ])
...
run_cov <- function(
  ...
  res <- t(vapply(
    ...
    function(i) {
      ...
      if (is.null(e)) {
        return(c(cov = NA, contain = NA, lo = NA, hi = NA, icc = NA))
      }
      c(
        cov = as.numeric(e$ci[1] <= icc_true && icc_true <= e$ci[2]),
  ...
  cat(sprintf(
    "  95%% MC-CI coverage of truth = %.3f  (target ~0.95)\n",
    mean(res[ok, "cov"])
  ))
  cat(sprintf(
    "  point-in-own-CI containment = %.3f\n",
    mean(res[ok, "contain"])
  ))
...
gammaB <- rnorm(8, 0, 1)
run_cov(
  sprintf("B interior: unequal k_c=%s", paste(kcB, collapse = ",")),
  Nc = 8,
  kc_vec = kcB,
  ns = 12,
  s2s = 1.0,
...
run_cov(
  "C boundary: theta^2_{r:c}=0 (equal rater means)",
  Nc = 8,
  kc_vec = kcB,
  ns = 12,
  s2s = 1.0,
