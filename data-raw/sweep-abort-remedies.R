# Does each reducer-stage abort's named `ci_method` actually work on the data
# that reaches that abort?
#
# PROVENANCE. Written for M100 on branch `m100-abort-remedy-truthfulness`
# (2026-08-01..03), which was dropped at the D-021 gate without merging; this file
# is that script, landed on the default branch by M103 so every usability verdict
# M103's guards rely on cites a committed, re-runnable artifact rather than a
# branch nothing protects. Two edits were made in landing it, both so it runs from
# a clean checkout:
#   1. The per-site leading lines below were READ from
#      `data-raw/abort-remedy-enumeration.txt`, an M100 artifact emitted by a
#      source-scanning enumerator that is not shipped (its class is the one D-021
#      closed). They are now inline literals, copied verbatim from that file.
#   2. The `named_by_remedy` column, which read the same artifact to report which
#      sites named a method at M100 time, is dropped. It was M100 reporting; no
#      M103 criterion reads it, and it would have gone stale the moment M103
#      re-named a method.
# The `site` keys are M100's own opaque enumerator hashes, kept verbatim so this
...
#
# Each leading line is the guard's own first message line, verbatim. Glue spans
# (`{.val {n_ok}}`) are cut out and the static text between them is what must
# appear -- ALL of it, not the longest piece, because " interval is undefined for
# this data." alone is shared by the classical and npbootstrap-observed guards and
# only "The classical one-way" separates them.
#
# M100 read these from its enumerator's output; see the provenance note at the top
# for why they are inline literals here. They are the four lines M103's AC1 and
# AC5 freeze, so a drift between this list and the shipped guard is a criterion
# failure there, not a silent mismatch here -- and it shows up in this script too,
# as a site that stops being reached by any cell.
enumeration_leads <- list(
  "R/ci-bootstrap.R:f3b08ac552" = paste(
