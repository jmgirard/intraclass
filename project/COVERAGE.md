...
| **incomplete-data** multilevel projection | ✅ subject level (M18 Slice 3; **crossed and nested** Designs 2/3 once incomplete nested ships, M19 Slice 1); cluster level dropped-with-note (bounded by the 🟣 Wave-3 `ICC(c,k)` incomplete divisor; nested designs have no cluster level). Projection moves only the divisor `m`, so the ragged subject fit projects unchanged (reduction to `ICC(A,k)` at `m = k_eff`; cross-engine; monotone/[0,1]). |
