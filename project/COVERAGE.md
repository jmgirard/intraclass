...
**conflated** diagnostic (`engine = "brms"` + `level = "conflated"`) now composes ten Hove Eq. 14 off the
crossed five-component posterior draws (a variance-ratio push-forward, no θ² moment correction); O-Bayes-
Conflated pins Eq-14 identity + coverage + glmmTMB containment. (Slice 2 — brms within-cell replicates —
pending.) Prior: **M27** (ADR-037) — Bayesian **multilevel fixed-rater**: `engine = "brms"` +
...
|---|---|---|---|---|---|
| **Design 1** crossed (5-component) | subject, cluster, conflated | agreement, consistency | random (both levels); fixed (subject only; balanced **and** incomplete) | balanced ✅; incomplete ✅\* | glmmTMB, lme4, **brms** (balanced; random subject+cluster — M24; **fixed subject — M27 Slice 1**; **conflated — M29 Slice 1**) |
