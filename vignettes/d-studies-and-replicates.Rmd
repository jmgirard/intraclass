...
--- each default to the value stored on the fit, so a seeded fit already gives a
reproducible projection. Pass any of them to `d_study()` to override it for this
call alone; `seed = 7` above does exactly that.
...
call rather than run it, since running it would render a second copy of the curve
above:
