...
`residual` (pure error) as separate terms. The **single-occasion** rows -- the
rows whose `occasions` column reads 1 -- are the ordinary ICCs, since a single
rating's error still includes the interaction. But they are now fit correctly,
rather than folding the interaction into the residual. The
**occasion-averaged** rows -- `occasions` 3 here, the fitted per-cell replicate
count -- give the reliability of a rater's *mean of three ratings*. Averaging
