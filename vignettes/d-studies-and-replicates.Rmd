...
definition**, adding a `type` column. That is handy when you want both, but for
...
(`occasions = 1`) are the ordinary ICCs, since a single rating's error still
includes the interaction. But they are now fit correctly, rather than folding the
interaction into the residual. The **occasion-averaged** rows (`occasions = 3` here)
give the reliability of a rater's *mean of three ratings*. Averaging cuts pure error
...
remains open is the occasion-*averaged* coefficient on ragged replicates, along with
the compound fixed-by-ragged and multilevel-by-ragged corners. With unequal per-cell
counts there is no single effective-occasion divisor with a validated oracle.
