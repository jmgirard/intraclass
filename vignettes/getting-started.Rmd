...
subject variation from rater variation as variance components), rather than from
...
on a small example: **fit → estimate → interpret**.
...
The interval `icc()` reports is a **Monte-Carlo** interval, and it is
**boundary-aware**. In plain terms: instead of relying on a textbook formula that
