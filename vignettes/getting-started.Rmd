...
`icc()` avoids the textbook formulas that misbehave when a variance is near
zero, a common situation, since raters who barely differ push the rater variance
to its zero boundary. It simulates many plausible parameter sets from the fitted
model instead, and reads the interval off the resulting spread of ICCs. This keeps the interval well-behaved right at that boundary. The [*Interval methods*](interval-methods.html) article covers how it
