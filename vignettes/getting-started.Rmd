...
raters: `ICC(A,1)`, `ICC(A,k)`, `ICC(C,1)`, `ICC(C,k)` (the next sections unpack
these labels). They are grouped by error definition in the printout. We set a `seed`
...
`summary()` is the other reading of the same fit: it reprints the report above and
then appends a short **interpretive note for each error definition present**, plus
one about what a single rating per cell can and cannot separate. Nothing is
recomputed --- the notes are read off the design, so they track whatever you fitted:
