...
Instead each subject may be rated by whichever raters happened to be available,
so that "rater 1" for one subject has nothing to do with "rater 1" for another.
Then the raters are *interchangeable* and the design is **one-way**
(`model = "oneway"`).
...
not apply to one-way, since there is no rater term to reason about. Answer this
question first.

## 1. Agreement vs. consistency (`type`)

**Does the actual value need to match, or only the rank order?** *Absolute
agreement* treats a systematic difference between raters as error, as when one
judge scores consistently higher than another. *Consistency* forgives a constant
...
anything, since there is no `data` argument. So it is a quick way to settle the
...
`choose_icc()` with them omitted to be asked each outstanding question in turn. Answering a choice that does not apply to your design (for example `type`
