...
away. (`irrICC` and the generalizability-theory package `gtheory` can also fit
incomplete data with their own models — see the capability matrix below — but the
...
Two rows deserve a word. The generalizability-theory tools — `gtheory`, and
model-based extractors such as `performance::icc` — return **variance
components** or a variance-partition coefficient, which is the raw material of an
ICC but not the inter-rater-reliability coefficient family itself, nor the
error-variance framing that distinguishes agreement from consistency.
(`intraclass`'s own generalizability coefficients are validated against `gtheory`
to within 0.001; see the package's reference notes.) And an **interval** that is
