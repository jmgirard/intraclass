...
on its design line, as `N cells x N replicates`, wherever the design line
carries one. A multilevel fit's design line reports subjects and clusters
instead, and shows no per-cell count anywhere, so read `n_o` for it there. See [*D-studies and within-cell
replicates*](d-studies-and-replicates.html).

## `occasions` vs. `n_o`

Two near-identical names for different quantities. `tidy()`'s `occasions`
column counts the **ratings averaged into that row's coefficient**: 1 for a
single-rating coefficient, the fitted per-cell count for an occasion-averaged
one, and `NA` when the design has no within-cell replicates. `glance()`'s `n_o`
counts the **occasions observed per cell** in the design that was fitted. So on
a design with three ratings per cell, a fit reporting both settings shows
`occasions` 1 and 3 down its rows, while `n_o` is 3 for the fit as a whole.
`n_o` is `NA` on a design that defines no single per-cell count. A ragged
replicate fit is the case that separates the two most sharply: `n_o` reads
`NA` there while `occasions` still reads 1.
