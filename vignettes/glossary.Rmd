...
on its design line, as `N cells x N replicates`; where `n_o` is `NA` the design
line reads `NA` in that slot too. A multilevel fit's design line reports
subjects and clusters instead, and shows no per-cell count anywhere, so read
`n_o` for it there. See [*D-studies and within-cell
replicates*](d-studies-and-replicates.html).

## `occasions` vs. `n_o`

Two near-identical names for different quantities. `tidy()`'s `occasions`
column is the **per-rater occasion divisor** that row's coefficient applies to
pure error. It reads 1 on every row that averages no occasions, which includes
every row whose error set carries no pure-error term to average. It reads the
fitted per-cell count where the row does average, and `NA` on a fit that splits
no within-cell replicates. `glance()`'s `n_o` counts the **occasions observed per cell** in
