...
interaction](#variance-component) from pure error. The per-cell count is
reported under two names for the same quantity: the printed report's design
line spells it as `N cells x N replicates`, and `glance()` reports that same
per-cell count in its `n_o` column. See [*D-studies and within-cell
replicates*](d-studies-and-replicates.html).
