...
so its width tracks the tail weight of the data — robust to non-normality where the
**exact-F interval** leans on it. Deterministic: no resamples, no seed. See
...
(Searle 1971; the McGraw & Wong 1996 Table 7 limits). Exact under normality —
best-calibrated and narrowest when the data are approximately normal — and
deterministic. See
[*Confidence-interval methods*](interval-methods.html#the-opt-in-boundary-robust-methods).
...
2.5% and 97.5% quantiles. Fast and boundary-aware. See [*Confidence-interval
methods*](interval-methods.html).
...
coefficient based on the restricted maximum likelihood estimator. *Journal of
Statistical Computation and Simulation, 81*(9), 1101--1115.
