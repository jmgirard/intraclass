...
re-explaining it. Terms are listed alphabetically. Nothing here is new: it is the
vocabulary the
...
**exact-F interval** leans on normality instead. Tracking tail weight is not the same as widening: on the two
...
`"burch"`'s narrower margin holds much the same up to a true ICC of 0.3 rather than shrinking as the true ICC rises
(on the larger grid; the smaller grid's margin does shrink across its levels).
The `"burch"` width advantage then collapses to near parity at a true ICC of 0.6, on the one grid reaching that value,
...
classrooms, patients in clinics). That is ten Hove et al.'s (2022) Equation 14. It
folds the between-cluster and within-cluster variation into one "true score" and is
...
single and average, subject level and cluster level are *different estimands*, so
...
under normality, meaning best-calibrated when the data are approximately normal, and
...
uses every observed value to estimate the model. See [*Estimation engines*](engines.html#a-structural-equation-engine-lavaan).
...
equals the random-rater variance. Under imbalance it differs. See **fixed vs. random raters**.
...
agreement**. A rater is a single column with no random effect, so its variance is
read from the spread of the estimated column (indicator) means (Jorgensen 2021). It
is a genuinely different estimator than the mixed model's random effect, though
asymptotically equivalent, and can differ modestly on small designs. See
...
result depends on. Overriding it (`prior =`) voids those guarantees, so `icc()` warns. See
...
reliably they distinguish *cluster means*. `icc()` reports both from one fit. See [*Multilevel
...
zero, the edge of the allowed range. Ordinary interval formulas misbehave there (they can
