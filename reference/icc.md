...
- ci_method:
  ...
  **non-parametric** transformed bootstrap-*t* of Ukoumunne et al.
  ...
  package has measured that vary only the subject effect, `"burch"` is
  usually the **narrower** of the two – but by a margin that depends on
  the design, so it is not a rule of thumb. That margin holds much the
  same up to a true ICC of 0.3 rather than shrinking as the true ICC
  ...
  0.6, on the one grid reaching that value, which is also where every
  cell `"searle"` won sits; and it shrinks steadily as the subject count
  grows, measured at 5 raters. What `"burch"` does against `"searle"`
  depends on what the residual is drawn from, and the three grids now
  measure that: the two grids that vary only the subject effect put it
  narrower nearly everywhere, while the third, which draws the residual
  ...
  at t(5) with 100 subjects) and narrower at every lighter-tailed one,
  the normal included. Treat neither as reliably the tighter interval;
  the per-level figures for the two grids that vary only the subject
  effect are tabulated in the interval-methods article. Burch's
  ...
  **only for the balanced-complete two-way random absolute-agreement
  ICC(A,1)** (with ICC(A,k) and any numeric-`unit` projection `ICC(A,m)`
...
- `tidy.icc()`: a tibble with one row per estimated coefficient carrying
  ...
  `std.error`, `conf.low`, `conf.high`, `conf.level`, and `method`, plus
  an `occasions` column – inserted after `index`, so the list above is
  not a column order – when the design has within-cell replicates.
...
- `autoplot.icc()`: a `ggplot` object – the coefficient forest plot, or
  the variance-component decomposition when `what = "components"`.
...
`"burch"`'s margin holds much the same up to a true ICC of 0.3 rather
...
every cell favouring `"searle"` sits. It shrinks steadily as the subject
...
included. Its robustness has a measured limit: on strongly skewed
subject effects `"burch"` under-covers about as badly as the default
(see the coverage caveat under Confidence intervals). Both share the
