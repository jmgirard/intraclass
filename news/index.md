...
- New `ci_method = "searle"` and `ci_method = "burch"` for the
  **balanced one-way random** design: two **deterministic classical
  ...
  interval of Burch (2011), wider and below-nominal in fewer cells (but
  see the correction above — it is not a remedy for heavy tails). Like
  `"npbootstrap"` they are **boundary robust** — a finite interval where
  the Monte-Carlo default aborts on near-zero-ICC data — and one-way
  only (they abort otherwise). Being closed forms they take no
  `mc_samples`, `boot_samples`, or `seed` and report no standard error;
  the `ICC(k)` interval is the exact Spearman-Brown image of the
